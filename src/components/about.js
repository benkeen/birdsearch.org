		...
		<Overlay id="about-overlay" showCloseIcon={true} onClose={this.close}>
			<div>
				<ul className="nav nav-pills">
					<li className={aboutClasses}><a href="#"
													onClick={(e) => this.selectTab(e, C.ABOUT_TABS.ABOUT)}><FormattedMessage
					id="about"/></a></li>
					<li className={historyClasses}><a href="#" onClick={(e) => this.selectTab(e, C.ABOUT_TABS.HISTORY)}><FormattedMessage
					id="history"/></a></li>
					<li className={translateClasses}><a href="#"
														onClick={(e) => this.selectTab(e, C.ABOUT_TABS.TRANSLATE)}><FormattedMessage
					id="translate"/></a></li>
					<li className={contactClasses}><a href="#" onClick={(e) => this.selectTab(e, C.ABOUT_TABS.CONTACT)}><FormattedMessage
					id="contact"/></a></li>
				</ul>
				{this.getTabContent()}
			</div>
...
class AboutTab extends React.Component {
	render() {
		...
		return (
		<div>
			<img src="/images/photos/sandhill.png" width="200" height="149" className="photo"
				 title="Sandhill crane, BC, Canada"/>
			<p>
				<FormattedMessage id="aboutPara1"
								  values={{
									  ...
									  settingsLink: <Link
									  to="/settings">{intl.formatMessage({ id: 'searchSettings' }).toLowerCase()}</Link>
								  }}/>
			</p>
			<p>
...
class HistoryTab extends React.Component {
	render() {
		...
		<div>
			...
			<p>
				<FormattedMessage id="historyPara2"
								  values={{ rightNow: <i>{intl.formatMessage({ id: 'rightNow' })}</i> }}/>
			</p>
		</div>
		);
	}
		...
		<div>
			...
			<p>
				<FormattedMessage id="translatePara1"
								  values={{
									  githubLink: <a
									  href="https://github.com/benkeen/birdsearch.org/tree/master/src/i18n"
									  target="_blank">{intl.formatMessage({ id: 'foundHere' })}</a>
								  }}/>
			</p>
		</div>
		);
		...
		<div>
			...
			<p>
				<FormattedMessage id="contactPara1"
								  values={{
									  postLink: <a href="https://github.com/benkeen/birdsearch.org/issues"
			...
			<p>
				<FormattedMessage id="contactPara2"
								  values={{
									  openSourceLink: <a href="http://github.com/benkeen/birdsearch.org"
														 target="_blank">{intl.formatMessage({ id: 'openSource' })}</a>
								  }}/>
			</p>
		</div>
		);
