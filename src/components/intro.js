...
class IntroOverlay extends React.Component {
	...
	render() {
		...
		<Overlay id="intro-overlay" className={classes} showCloseIcon={true} onClose={this.close}
				 outerHTML={this.getLoader()}
				 customKeyActions={this.getCustomKeyActions()}>
			<div>
				<button className="btn btn-success" id="searchNearby" onClick={this.searchNearby}
						disabled={loading || !this.state.supportsGeoLocation}>
			...
			<div>
				<button className="btn btn-primary" id="searchAnywhere" onClick={this.searchAnywhere}
						disabled={loading}>
